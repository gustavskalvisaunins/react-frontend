...
import logo from '../../assets/pineapple-logo.svg';
import './Overlay.css';
...
function Overlay () {
    ...
    const [emailClassName, setEmailClassName] = useState('email-container');
    const [lineClassName, setLineClassName] = useState('color-line');
    const [isIconVisible, setIsIconVisible] = useState(false);
    const [isInputVisible, setIsInputVisible] = useState(true);
    const [headline, setHeadline] = useState('Subscribe to newsletter');
    const [description, setDescription] = useState('Subscribe to our newsletter and get 10% discount on pineapple glasses.');
    ...
    useEffect(()=> {
        if (email !== null || checked !== false) {
            if (email === '' || !regEx.test(email) || email.slice(-3) === '.co' || checked === false) {
                setIsDisabled(true);
                setEmailClassName('email-container-error');
                setLineClassName('color-line-error');

                if (email === '') {
                    setError('E-mail address is required');
                } else if (!regEx.test(email) && email !== '') {
                    setError('Please provide a valid e-mail address');
                } else if (email.slice(-3) === '.co') {
                    setError('We are not accepting subscriptions from Colombia e-mails');
                ...
                setError('');
                setIsDisabled(false);
                setEmailClassName('email-container');
                setLineClassName('color-line');
            }
        }
    }, [email, checked]);

    const handleSubmit = () => {
        setIsIconVisible(current => !current);
        setIsInputVisible(current => !current);
        setHeadline('Thanks for subscribing!');
        setDescription('You have successfully subscribed to our email listing. Check your email for the discount code.');
    };
    ...
    return (
        <section className="overlay">
            ...
            <div className="content">
                ...
                {/* Success icon */}
                {isIconVisible === true &&
                    <svg className="success-icon" width="44" height="70" viewBox="0 0 44 70" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        <div className={emailClassName}>
                            ...
                            >
                                <svg width="24" height="14" viewBox="0 0 24 14" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path className="arrow-icon" d="M17.7071 0.2929C17.3166 -0.0976334 16.6834 -0.0976334 16.2929 0.2929C15.9023 0.683403 15.9023 1.31658 16.2929 1.70708L20.5858 5.99999H1C0.447693 5.99999 0 6.44772 0 6.99999C0 7.55227 0.447693 7.99999 1 7.99999H20.5858L16.2929 12.2929C15.9023 12.6834 15.9023 13.3166 16.2929 13.7071C16.6834 14.0976 17.3166 14.0976 17.7071 13.7071L23.7071 7.70708C24.0977 7.31658 24.0977 6.6834 23.7071 6.2929L17.7071 0.2929Z"/>
                                </svg>
                            </button>
                        </div>
